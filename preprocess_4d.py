...
class US4DData:
    ...
    data: np.ndarray          # shape (nx, ny, nz, nt), uint8
    scale: list[float]        # [scale_x, scale_y, scale_z] in mm/pixel
    dim: list[float]          # [size_x, size_y, size_z] in mm
    N: list[int]              # [nx, ny, nz]
    frame_rate: float = 15.0  # volumes / second
...
def explore_dicom(path: str | Path) -> None:
    """Print all DICOM tags, flag dimension/spacing-related ones."""
    ...
    for elem in ds:
        ...
        if any(k in name_lower for k in dim_keywords) or elem.tag.group == 0x3001:
            val = ("[pixel data]" if elem.tag.group == 0x7FE0
                   else elem.value)
...
def load_4d(path: str | Path, frame_rate: float = 15.0) -> US4DData:
    """
    Load a Philips 4D echo DICOM and return a US4DData struct.

    Layout in file (inferred from private tags):
        raw bytes → (nt * nz, ny, nx) → reshape → (nt, nz, ny, nx)
        then transpose to (nx, ny, nz, nt)  [x=cols, y=rows, z=depth, t=time]
    """
    ds = pydicom.dcmread(str(path), force=True)

    # ── spatial dimensions ────────────────────
    ny = int(ds.Rows)           # rows    (y)
    nx = int(ds.Columns)        # columns (x)
    nz = int(ds[0x3001, 0x1001].value)   # depth from private Philips tag

    total_bytes = len(ds.PixelData)
    nt = total_bytes // (nx * ny * nz)
    assert nx * ny * nz * nt == total_bytes, (
        f"Pixel data size {total_bytes} not divisible by nx*ny*nz = {nx*ny*nz}")

    # ── voxel spacing (cm → mm) ───────────────
    dx = float(ds[0x0018, 0x602C].value) * 10.0   # Physical Delta X
    dy = float(ds[0x0018, 0x602E].value) * 10.0   # Physical Delta Y
    dz = float(ds[0x3001, 0x1003].value) * 10.0   # private Philips Z delta

    print(f"[load_4d] Spatial dims  : nx={nx}, ny={ny}, nz={nz}, nt={nt}")
    print(f"[load_4d] Voxel spacing : dx={dx:.4f} mm, dy={dy:.4f} mm, dz={dz:.4f} mm")

    # ── decode raw bytes → uint8 array ────────
    raw = np.frombuffer(ds.PixelData, dtype=np.uint8)

    # Stored as (nt*nz) 2-D frames, each (ny, nx), time-major
    vol = raw.reshape((nt, nz, ny, nx))   # (t, z, y, x)
    vol = vol.transpose(3, 2, 1, 0)       # → (x, y, z, t)  [MATLAB-like order]
...
def plot_orthogonal_slices(us: US4DData, t: int = 0,
    ...
    """Three centre orthogonal slices at time t."""
    vol = us.data[:, :, :, t]           # (nx, ny, nz)
    ...
    slices = [
        vol[cx, :, :].T,   # (ny, nz) → imshow as (row=nz, col=ny)
        vol[:, cy, :].T,   # (nx, nz)
        vol[:, :, cz].T,   # (nx, ny)
    ]
...
def save_time_animation(us: US4DData, axis: str = "z",
    ...
    """
    Save a GIF of the time series for the centre slice along `axis`
    ('x', 'y', or 'z').
    """
...
def save_nifti_slice(us: US4DData, z: int,
    ...
    """
    Save a single z-slice as a 4-D NIfTI (nx, ny, 1, nt) for playback in
    3D Slicer via the Sequences module.
    """
    import nibabel as nib

    # (nx, ny, nt) → add singleton z axis → (nx, ny, 1, nt)
    ...
    affine = np.array([
        [dx,  0,  0,  0],
        [ 0, dy,  0,  0],
        [ 0,  0,  1,  0],   # z spacing = 1 mm (singleton slice)
        [ 0,  0,  0,  1],
    ])
...
def save_nifti(us: US4DData, out_path: str | Path = "us4d.nii.gz") -> None:
    """
    Save 4-D ultrasound data as a NIfTI-1 file (.nii.gz).

    NIfTI convention used here:
      - axes: (x, y, z, t)  — matches us.data layout directly
      - qform/sform set to a diagonal affine with voxel spacing in mm
      - dim_info encodes freq/phase/slice
      - pixdim[4] encodes temporal spacing (1/frame_rate seconds)
      - intent_code 0  (NONE — raw intensity, suitable as segmentation input)
    """
    import nibabel as nib

    # NIfTI expects (i, j, k, t) with i/j/k = x/y/z
    # us.data is already (nx, ny, nz, nt) — use directly
    data = us.data.astype(np.uint8)

    dx, dy, dz = us.scale          # mm/pixel
    dt = 1.0 / us.frame_rate       # seconds/volume

    affine = np.diag([dx, dy, dz, dt, 1.0])[:4, :4]
    # Full 4×4 affine: voxel → mm (origin at [0,0,0])
    affine = np.array([
        [dx,  0,  0,  0],
        [ 0, dy,  0,  0],
        [ 0,  0, dz,  0],
    ...
    hdr = img.header
    hdr.set_xyzt_units(xyz="mm", t="sec")
    hdr["pixdim"][4] = dt          # temporal voxel size
    hdr["dim_info"] = 0
    hdr["scl_slope"] = 1.0
    hdr["scl_inter"] = 0.0
